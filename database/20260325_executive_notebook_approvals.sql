-- Allow executive users to review notebook lending requests without granting full notebook inventory management.

create or replace function public.is_notebook_approver()
returns boolean
language sql
stable
security definer
...
grant execute on function public.is_notebook_approver() to authenticated;
...
as $$
begin
  if not public.is_notebook_approver() then
...
begin
  ...
  perform public.send_chat_message_to_user(
    _log.user_id,
    format(
      'Notebook %s approved for borrowing',
