...
export default function StockManagementPage({ theme, uiTheme, currentUser, stockManagementSection = "issue", onStockManagementSectionChange }) {
  ...
  const handlePrefillIssue = (item) => {
    ...
    window.requestAnimationFrame(() => issueFormRef.current?.scrollIntoView?.({ behavior: "smooth", block: "start" }));
  };
  ...
  return (
    <>
      ...
      {showIssuePage ? (
        <section className="mt-4">
          <article ref={issueFormRef} onPaste={handlePasteIssueEvidence} className={`rounded-lg border p-4 ${uiTheme.surfaceCard}`}>
